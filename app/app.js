...
function table(rows, cols) {
  if (!rows || !rows.length) return '<div class="state">No data.</div>';
...
async function load() {
  ...
  // Period filter buttons (All Time + each month)
  const periods = [{ ym: null, label: 'All Time' }, ...(months || []).map((m) => ({ ym: m.ym, label: m.label }))];
  const bar = document.getElementById('period-bar');
  bar.innerHTML = periods.map((p, i) =>
    `<button class="period-btn${i === 0 ? ' active' : ''}" data-ym="${p.ym ?? ''}">${p.label}</button>`).join('');
  bar.querySelectorAll('.period-btn').forEach((btn) => btn.addEventListener('click', () => {
    bar.querySelectorAll('.period-btn').forEach((b) => b.classList.remove('active'));
    btn.classList.add('active');
    loadRankings(btn.dataset.ym || null);
  }));

  // ---- Rankings (period-aware) ----
